...
###The Mr. Trash Wheel dataset contains `r length(mr_trash_wheel)` variables including: dumpster, month, year, date, weight (tons), etc. from May of 2014 until July of 2018. It is contains `r nrow(mr_trash_wheel)` rows and `r ncol(mr_trash_wheel)` columns for a total of `r nrow(mr_trash_wheel)*ncol(mr_trash_wheel)` observations The median number of sports balls in a dumpster in 2017 was `r median(pull(mr_trash_wheel_2017, sports_balls))`.

###The combined precipition dataset contains the total preciptation per month for the year 2017 and half of the  year 2018. It contains `r nrow(precip_data)` rows and `r ncol(precip_data)` columns for a total of `r nrow(precip_data)*ncol(precip_data)` observations. In 2018 the total precipitation was `r sum(pull(precip_data_2018, total))`. 
