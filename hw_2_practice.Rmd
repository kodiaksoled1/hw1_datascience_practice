...
## _Description_
### The Mr. Trash Wheel dataset contains `r nrow(mr_trash_wheel)` observations and `r ncol(mr_trash_wheel)` variables for a total of `r nrow(mr_trash_wheel)*ncol(mr_trash_wheel)` cells. The `r length(mr_trash_wheel)` variables include: dumpster, month, year, date, weight (tons), volume (cubic yards), etc. from May 2014 until June 2019. The median number of sports balls in a dumpster in 2017 was `r median(pull(mr_trash_wheel %>% filter(year == "2017"), sports_balls))`.

### The combined precipition dataset contains the total preciptation per month for the years 2017 and 2018. It contains `r nrow(precip_data)` observations and `r ncol(precip_data)` variables (month, year, and total precipitation) for a total of `r nrow(precip_data)*ncol(precip_data)` cells. In 2018 the total precipitation was `r sum(pull(precip_data %>% filter(year == "2018"), total))`. 

# *Problem 2* 

## _FiveThiryEight_

### First, we can import (`read_csv`) and clean (`janitor`) the three datasets: pols-month.csv, unemployment.csv, and snp.csv. Then, we can use the `separate` function to change the "date" variable into three varaibles ("year", "month", and "day") for the pols-month and snp datasets (_note: this isn't necessary for the unemployment dataset as it was already seperated by date_). 

### Specifically for the pols-month data, we can replace the month number with the abbreviated month name (to match the unemployment dataset that contains abbreviated month names) with `month.abb` under the `mutate` function, after specifying the month as an integer (`as.integer`). We also can create a new variable "president" under `mutate` with the `if_else` function and remove the "prez_gap", "prez_day", and "day" variables with `select` to clean up the dataset. 
...
### The pols-month dataset contained `r nrow(pols_month)*ncol(pols_month)` cells (`r nrow(pols_month)` observations and `r ncol(pols_month)` variables) about the republican and democrat president, govenor, senator, and house representative from 1947-2015. The snp dataset contained `r nrow(snp)*ncol(snp)` cells (`r nrow(snp)` observations and `r ncol(snp)` varaibles) about the closing rate of the snp from 1950-2015. The unemployment dataset contained `r nrow(unemployment)*ncol(unemployment)` cells (`r nrow(unemployment)` observations and `r ncol(unemployment)` variables) about the umployment rate by month from 1948-2015.

### The combined dataset contains data from 1947 to 2015 on the variables: month, year, president, unemployment rate, the closing rate for the snp, and the govenor, senator, house of representatives of the democratic and republican parties. It is contains `r nrow(pol_snp_unemployment_data)` rows and `r ncol(pol_snp_unemployment_data)` columns for a total of `r nrow(pol_snp_unemployment_data)*ncol(pol_snp_unemployment_data)` observations. 
