...
## _Description_
### The Mr. Trash Wheel dataset contains `r nrow(mr_trash_wheel)` rows/observations and `r ncol(mr_trash_wheel)` columns/variables for a total of `r nrow(mr_trash_wheel)*ncol(mr_trash_wheel)` cells. THe `r length(mr_trash_wheel)` variables include: dumpster, month, year, date, weight (tons), volume (cubic yards), etc. from May 2014 until June 2019. The median number of sports balls in a dumpster in 2017 was `r median(pull(mr_trash_wheel %>% filter(year == "2017"), sports_balls))`.

### The combined precipition dataset contains the total preciptation per month for the years 2017 and 2018. It contains `r nrow(precip_data)` rows/observations and `r ncol(precip_data)` columns/variables for a total of `r nrow(precip_data)*ncol(precip_data)` cells. In 2018 the total precipitation was `r sum(pull(precip_data %>% filter(year == "2018"), total))`. 
